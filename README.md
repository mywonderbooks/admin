...
const Icons = {
  ...
  Speed: ({ className }) => <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></svg>,
  Smile: ({ className }) => <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}><circle cx="12" cy="12" r="10"></circle><path d="M8 14s1.5 2 4 2 4-2 4-2"></path><line x1="9" y1="9" x2="9.01" y2="9"></line><line x1="15" y1="9" x2="15.01" y2="9"></line></svg>,
  PenTool: ({ className }) => <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}><path d="M12 19l7-7 3 3-7 7-3-3z"></path><path d="M18 13l-1.5-7.5L2 2l3.5 14.5L13 18l5-5z"></path><path d="M2 2l7.586 7.586"></path><circle cx="11" cy="11" r="2"></circle></svg>
};
...
const INITIAL_BOOKS = [
  {
    ...
    pages: [
      ...
      { 
        text: {
          English: "He would sneak into the kitchen to find sticky candies. It was his favorite secret mission of the day.",
          Spanish: "Se escabullía en la cocina para encontrar dulces pegajosos. Era su misión secreta favorita del día.",
        },
        emoji: "🍬"
      },
      { 
        text: {
          English: "But his mom warned him that sugar bugs love dirty teeth. She said they throw parties in your mouth if you don't brush!",
          Spanish: "Pero su mamá le advirtió que a los bichos del azúcar les encantan los dientes sucios. ¡Dijo que hacen fiestas en tu boca si no te cepillas!",
        },
        emoji: "🦠"
      },
      {
        text: {
            English: "One day, T-Rex woke up with a big toothache. 'Ouch!' he cried, holding his jaw.",
            Spanish: "Un día, T-Rex se despertó con un gran dolor de muelas. '¡Ay!' gritó, sosteniéndose la mandíbula."
        },
        emoji: "🤕"
      }
    ]
...
const BookReader = ({ book, initialLanguage = "English", onBack }) => {
  ...
  // Split text into sentences for highlighting simulation
  const sentences = (currentPage.text[currentLang] || currentPage.text['English']).match(/[^\.!\?]+[\.!\?]+/g) || [currentPage.text[currentLang]];
  ...
  const speakText = () => {
    ...
    utterance.lang = currentLang === 'Spanish' ? 'es-ES' : 'en-US'; // Basic mapping
    
    // Simulate sentence highlighting
    utterance.onboundary = (event) => {
        // Very basic estimation of sentence progress
        // Real sentence-level sync requires more complex logic or backend timestamps
    };

    utterance.onstart = () => {
        setIsSpeaking(true);
        setHighlightedSentence(0); // Start highlighting first block
    };
  ...
  useEffect(() => {
    if (isSpeaking) {
        speakText(); // Re-trigger if page changes while speaking
    }
  }, [pageIndex]);

  useEffect(() => {
    if (isSpeaking) {
        speakText(); // Re-trigger if speed/lang changes
    }
  ...
  if (showFinishScreen) {
    return (
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4">
            <div className="bg-white rounded-3xl p-8 max-w-md w-full text-center animate-in zoom-in">
                <div className="text-6xl mb-4">🏆</div>
                <h2 className="text-3xl font-bold text-slate-800 mb-2">Book Completed!</h2>
                <p className="text-slate-500 mb-8">You finished {book.title}. Great job!</p>
                <div className="flex flex-col gap-3">
                    <button onClick={() => { setShowFinishScreen(false); setPageIndex(0); }} className="w-full bg-indigo-600 text-white py-3 rounded-xl font-bold hover:bg-indigo-700">Read Again</button>
                    <button onClick={() => { setShowFinishScreen(false); setShowQuiz(true); }} className="w-full bg-amber-400 text-white py-3 rounded-xl font-bold hover:bg-amber-500">Take Quiz</button>
                    <button onClick={onBack} className="w-full bg-slate-100 text-slate-600 py-3 rounded-xl font-bold hover:bg-slate-200">Go Home</button>
                </div>
            </div>
        </div>
    );
  ...
  return (
    <div className="fixed inset-0 bg-[#F0F2F5] flex flex-col items-center justify-center font-sans">
      
      {/* MAIN CONTENT AREA */}
      <div className="flex-1 w-full max-w-6xl p-6 flex gap-6 items-center h-[calc(100vh-100px)]">
        
        {/* LEFT: IMAGE */}
        <div className="flex-1 bg-white h-full rounded-[2rem] shadow-sm border-2 border-slate-100 flex items-center justify-center relative overflow-hidden">
             <div className="text-[12rem] animate-in zoom-in duration-700">{currentPage.emoji}</div>
        </div>

        {/* RIGHT: TEXT */}
        <div className="flex-1 bg-white h-full rounded-[2rem] shadow-sm border-2 border-slate-100 flex items-center justify-center p-12 relative">
            <div className="text-3xl leading-relaxed font-medium text-slate-700 text-center">
                {sentences.map((sentence, idx) => (
                    <span 
                        key={idx} 
                        className={`transition-colors duration-300 ${isSpeaking && idx === highlightedSentence ? 'bg-indigo-100 text-indigo-700 rounded px-1 box-decoration-clone' : ''}`}
                    >
                        {sentence} 
                    </span>
                ))}
      ...
      {/* BOTTOM CONTROL BAR */}
      <div className="h-[100px] w-full bg-white border-t border-slate-200 flex items-center justify-between px-8 relative z-10 shadow-[0_-5px_20px_rgba(0,0,0,0.05)]">
         
         {/* CENTER PLAY BUTTON (OVERLAPPING) */}
         <div className="absolute left-1/2 -translate-x-1/2 -top-8">
            <button 
                onClick={toggleSpeech}
                className="w-20 h-20 bg-[#6366F1] hover:bg-[#4F46E5] rounded-full shadow-xl flex items-center justify-center text-white transition-transform hover:scale-105 active:scale-95 border-4 border-white"
            >
                {isSpeaking ? <Icons.Pause className="w-8 h-8 fill-current" /> : <Icons.Play className="w-8 h-8 fill-current ml-1" />}
            </button>
         </div>

         {/* LEFT CONTROLS */}
         <div className="flex items-center gap-4 w-1/3">
            <button onClick={onBack} className="flex items-center gap-2 px-4 py-2 rounded-full border border-slate-200 hover:bg-slate-50 text-slate-600 font-bold text-sm">
                <Icons.X className="w-4 h-4" /> Exit
            </button>
            
            <div className="relative group">
                <button className="flex items-center gap-2 px-4 py-2 rounded-full border border-slate-200 hover:bg-slate-50 text-slate-600 font-bold text-sm">
                    <Icons.Speed className="w-4 h-4" /> {speed}x
                </button>
                <div className="absolute bottom-full left-0 mb-2 bg-white rounded-xl shadow-lg border border-slate-100 p-2 hidden group-hover:block w-32">
                    {SPEEDS.map(s => (
                        <button key={s} onClick={() => setSpeed(s)} className={`block w-full text-left px-3 py-2 rounded-lg text-sm ${speed === s ? 'bg-indigo-50 text-indigo-600 font-bold' : 'hover:bg-slate-50'}`}>
                            {s}x Speed
                        </button>
                    ))}
                </div>
            ...
            <div className="relative group">
                <button className="flex items-center gap-2 px-4 py-2 rounded-full border border-slate-200 hover:bg-slate-50 text-slate-600 font-bold text-sm min-w-[120px] justify-between">
                    <div className="flex items-center gap-2"><Icons.Globe className="w-4 h-4" /> {currentLang}</div>
                    <Icons.ChevronUp className="w-3 h-3" />
                </button>
                <div className="absolute bottom-full left-0 mb-2 bg-white rounded-xl shadow-lg border border-slate-100 p-2 hidden group-hover:block w-48 max-h-64 overflow-y-auto">
                    {LANGUAGES.map(l => (
                        <button key={l} onClick={() => setCurrentLang(l)} className={`block w-full text-left px-3 py-2 rounded-lg text-sm ${currentLang === l ? 'bg-indigo-50 text-indigo-600 font-bold' : 'hover:bg-slate-50'}`}>
                            {l}
                        </button>
                    ))}
                </div>
         ...
         {/* CENTER THUMBNAILS */}
         <div className="flex items-center gap-3 justify-center w-1/3 pt-4">
             {book.pages.map((p, idx) => (
                 <button 
                    key={idx}
                    onClick={() => { setPageIndex(idx); setIsSpeaking(false); window.speechSynthesis.cancel(); }}
                    className={`w-12 h-10 rounded-lg border-2 flex items-center justify-center text-lg transition-all ${idx === pageIndex ? 'border-[#6366F1] bg-indigo-50 -translate-y-2' : 'border-slate-200 bg-slate-50 hover:border-slate-300'}`}
                 >
                    {p.emoji}
                 </button>
             ))}
         ...
         <div className="flex items-center justify-end gap-3 w-1/3">
             <button 
                ...
                className="w-10 h-10 rounded-full bg-slate-100 hover:bg-slate-200 disabled:opacity-50 flex items-center justify-center text-slate-600"
             >
                <Icons.ArrowLeft className="w-5 h-5" />
             </button>
             <button 
                onClick={() => setPageIndex(p => Math.min(book.pages.length - 1, p + 1))}
             ...
             <button 
                ...
                className="bg-[#FDE68A] hover:bg-[#FCD34D] text-amber-900 px-5 py-2 rounded-full font-bold flex items-center gap-2 shadow-sm shadow-amber-100"
             >
                <Icons.Brain className="w-4 h-4" /> Quiz
             </button>
         </div>
      </div>
    </div>
        ...
        <div className="min-h-screen bg-white font-sans text-slate-900">
            ...
            <div className="max-w-7xl mx-auto px-8 py-6 flex justify-between items-center">
                <button onClick={onBack} className="text-slate-500 hover:text-slate-800 font-bold flex items-center gap-2">
                    <Icons.ArrowLeft className="w-5 h-5" /> Back to Library
                </button>
                <div className="flex items-center gap-4">
                    <div className="bg-amber-100 text-amber-700 px-3 py-1 rounded-full text-sm font-bold flex items-center gap-1">
                        <Icons.Star className="w-4 h-4 fill-current" /> 120
                    </div>
                    <div className="w-10 h-10 bg-indigo-100 rounded-full flex items-center justify-center text-indigo-600 font-bold">Leo</div>
                </div>
            </div>

            {/* Main Content */}
            <div className="max-w-5xl mx-auto px-8 py-8 flex flex-col md:flex-row gap-12">
                
                {/* Left: Book Card */}
                <div className="w-full md:w-1/3">
                    <div className={`aspect-[3/4] ${book.coverColor} rounded-[2.5rem] flex flex-col items-center justify-center shadow-2xl relative p-8`}>
                        <div className="absolute top-6 right-6 bg-white/30 backdrop-blur-md text-slate-800 px-3 py-1 rounded-full text-xs font-bold uppercase">{book.level}</div>
                        <div className="text-[10rem] drop-shadow-2xl">{book.coverEmoji}</div>
                    </div>
                </div>
                ...
                <div className="flex-1 space-y-8">
                    <div className="flex justify-between items-start">
                        <h1 className="text-5xl font-extrabold text-slate-900 leading-tight">{book.title}</h1>
                        <button className="p-3 bg-pink-50 text-pink-500 rounded-full hover:bg-pink-100 transition-colors">
                            <Icons.Heart className="w-8 h-8" />
                        </button>
                    </div>

                    <div className="flex items-center gap-6 text-sm font-bold text-slate-500">
                        <div className="flex items-center gap-1 text-slate-900"><Icons.Star className="w-5 h-5 text-amber-400 fill-current" /> 4.8</div>
                        ...
                        <div className="flex-1">
                            ...
                            <div className="relative">
                                <select 
                                    ...
                                    className="w-full bg-white border border-slate-200 text-slate-800 font-bold py-3 pl-4 pr-10 rounded-xl appearance-none focus:outline-none focus:ring-2 focus:ring-indigo-500"
                                >
                                    {LANGUAGES.map(l => <option key={l} value={l}>{l}</option>)}
                                </select>
                                <Icons.ChevronDown className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 w-4 h-4 pointer-events-none" />
                            </div>
                        </div>
                        <button 
                            onClick={() => onPlay(selectedLang)}
                            className="flex-[2] bg-[#6366F1] hover:bg-[#4F46E5] text-white text-lg font-bold py-4 rounded-xl shadow-lg shadow-indigo-200 hover:shadow-xl hover:-translate-y-0.5 transition-all flex items-center justify-center gap-3"
                        >
                            <Icons.Play className="w-6 h-6 fill-current" /> Play Now
                        </button>
                    </div>

                    <div className="grid grid-cols-2 gap-4">
                        <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 flex items-center gap-3">
                            <div className="w-10 h-10 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center"><Icons.PenTool className="w-5 h-5" /></div>
                            <div>
                                <div className="text-xs font-bold text-slate-400 uppercase">Author</div>
                                <div className="font-bold text-slate-800">{book.author}</div>
                            </div>
                        </div>
                        <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 flex items-center gap-3">
                            <div className="w-10 h-10 bg-purple-100 text-purple-600 rounded-full flex items-center justify-center"><Icons.Smile className="w-5 h-5" /></div>
                            <div>
                                <div className="text-xs font-bold text-slate-400 uppercase">Illustrator</div>
                                <div className="font-bold text-slate-800">{book.illustrator || "Unknown"}</div>
                            </div>
                        </div>
                    </div>

                    <div>
                         <label className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3 block">Rate This Story</label>
                         <div className="flex gap-2">
                             {[1,2,3,4,5].map(i => (
                                 <Icons.Star key={i} className="w-8 h-8 text-slate-200 hover:text-amber-400 cursor-pointer transition-colors" />
                             ))}
                         </div>
                    </div>

                    <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100">
                         <label className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 block">Story Summary</label>
                         <p className="text-slate-600 leading-relaxed font-medium">"{book.description}"</p>
                    </div>
                </div>
            </div>
        </div>
...
const App = () => {
  ...
  // Safe handler to pass to StoryDetails
  const handlePlayBook = (lang) => {
    setView('reader');
    // We can pass the selected language to the reader here if we added a prop for it
    // For now, Reader defaults to English but has a switcher.
    // Ideally: <BookReader initialLanguage={lang} ... />
  };
  ...
  return (
    <div className="h-screen bg-white font-sans text-gray-800 overflow-hidden">
      ...
      {view === 'details' && selectedBook && <StoryDetailsScreen book={selectedBook} onPlay={handlePlayBook} onBack={() => setView('library')} />}
